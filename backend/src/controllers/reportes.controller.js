...
const { logQuery, userTag } = require('../config/logger')
...
const getCentral = async (req, res, next) => {
  try {
    ...
    logQuery(`📊 DESCARGAR reporte central año=${anio} — ${userTag(req.user)}`)
...
const getDistrito = async (req, res, next) => {
  try {
    ...
    logQuery(`📊 DESCARGAR reporte distrito=${idDistrito} año=${anio} — ${userTag(req.user)}`)
...
const getPendientes = async (req, res, next) => {
  try {
    ...
    const anio      = req.query.anio ? parseInt(req.query.anio) : new Date().getFullYear()
    const { idDistrito } = req.user
    logQuery(`📊 DESCARGAR pendientes año=${anio} — ${userTag(req.user)}`)
...
const getHistorico = async (req, res, next) => {
  try {
    ...
    const anio      = req.query.anio       ? parseInt(req.query.anio)       : new Date().getFullYear()
    const { idDistrito } = req.user
    logQuery(`📊 DESCARGAR histórico año=${anio} meses=${mesInicio}-${mesFin} — ${userTag(req.user)}`)
...
const getAvanceDistritosExcel = async (req, res, next) => {
  try {
    ...
    const anio = req.query.anio ? parseInt(req.query.anio) : new Date().getFullYear()
    const { perfil, clave } = req.user
    logQuery(`📊 DESCARGAR avance distritos Excel año=${anio} — ${userTag(req.user)}`)
...
const getAvanceActividadesExcel = async (req, res, next) => {
  try {
    ...
    const anio = req.query.anio ? parseInt(req.query.anio) : new Date().getFullYear()
    const { perfil, clave } = req.user
    logQuery(`📊 DESCARGAR avance actividades Excel año=${anio} — ${userTag(req.user)}`)
...
const getACapturar = async (req, res, next) => {
  try {
    ...
    const anio = req.query.anio ? parseInt(req.query.anio) : new Date().getFullYear()
    const { idDistrito } = req.user
    logQuery(`📊 DESCARGAR a-capturar año=${anio} — ${userTag(req.user)}`)
...
const descargarHistorico = async (req, res, next) => {
  try {
    ...
    logQuery(`📁 DESCARGAR histórico archivo año=${anio} — ${userTag(req.user)}`)
