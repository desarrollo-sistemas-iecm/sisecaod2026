...
const getAll = async (req, res, next) => {
  try {
    ...
    logQuery(`📋 Consulta catálogo — ${userTag(req.user)}`)
...
const getById = async (req, res, next) => {
  try {
    ...
    logQuery(`🔍 Consulta actividad id=${id} — ${userTag(req.user)}`)
    const data = await catalogoService.getById(id, { perfil, idDistrito, clave })
...
const create = async (req, res, next) => {
  try {
    ...
    logAction(`➕ CREAR actividad clave=${req.body.clave} — ${userTag(req.user)}`)
...
const update = async (req, res, next) => {
  try {
    const id = parseInt(req.params.id)
    await catalogoService.update(id, req.body, req.user)
    logAction(`✏️  EDITAR actividad id=${id} clave=${req.body.clave} — ${userTag(req.user)}`)
...
const remove = async (req, res, next) => {
  try {
    ...
    logAction(`🗑️  ELIMINAR actividad id=${id} — ${userTag(req.user)}`)
...
const importar = async (req, res, next) => {
  try {
    ...
    logAction(`📥 IMPORTAR catálogo — ${result.insertados} actividades — ${userTag(req.user)}`)
...
const importarDesfase = async (req, res, next) => {
  try {
    ...
    logAction(`📥 IMPORTAR DESFASE — mes=${mesReal} año=${anoReal} — ${result.insertados} act — ${userTag(req.user)}`)
